...
SortAnimate.prototype = {
  ...
  getColumn: function (number) {
    return `<div class="column" style="height:${this.getHeight(
      number
    )}%" title="${number}"  >
  ...
  uiSetHeight: function (index) {
    ...
    this.ui.columns[index].firstElementChild.innerHTML = number;
  ...
  uiUnHighlightOne: function (index1, index2) {
    this.ui.columns[index1].className = this.ui.columns[
      index1
    ].className.replace(" highlight", "");
    this.ui.columns[index2].className = this.ui.columns[
      index2
    ].className.replace(" highlight", "");
  },
  //激活第 index 个元素
  ...
  uiBlurOne: function (index) {
    this.ui.columns[index].className = this.ui.columns[index].className.replace(
      " focus",
      ""
    );
  },
  ...
  uiBlurFragment: function (startIndex, endIndex) {
    ...
    for (var i = startIndex; i < endIndex; i++) {
      this.ui.columns[i].className = this.ui.columns[i].className.replace(
        " active",
        ""
      );
    }
